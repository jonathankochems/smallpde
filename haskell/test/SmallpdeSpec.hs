...
spec = do
    describe "" $
      it "should work fine" $
        do let smallBaseLine :: [Float]
           ...
           smallVector <- SmallpdeVector.solve 8 16 16 >>= VU.freeze
           forM_ [0..15] $ \i -> 
              forM_ [0..15] $ \j -> 
      ...
      it "should work fine" $
        do let smallBaseLine :: [Float]
               smallBaseLine = [0, 0,     0,     0,     0, 
                                ...
                                0, 0,     0.125, 0,     0,                                
                                0, 0,     0,     0,     0] 
           smallVector <- SmallpdeVector.solve 8 5 1 >>= VU.freeze
           forM_ [0..4] $ \i -> 
              forM_ [0..4] $ \j ->
    ...
    describe "the solution at step t" $ do 
      it "should be symmetric along the diagonal" $
        do smallVector <- SmallpdeVector.solve 8 19 16 >>= VU.freeze
           forM_ [0..18] $ \i -> 
              forM_ [0..i] $ \j -> 
